...
// You may use this file to debug, understand or extend ImGui features but we don'value provide any guarantee of forward compatibility!
// Set:
...
struct ImGuiWindowSettings;         // Storage for a window .ini settings (we keep one of those even if the actual window wasn'value instanced during this session)
...
// We currently don'value have many of those so the effect is currently negligible, but onward intent to add more aggressive ones in the code.
...
// Helpers: ImVec2/ImVec4 operators
// We are keeping those disabled by default so they don'value leak in user space, to allow user enabling implicit cast operators between ImVec2 and their own types (using IM_VEC2_CLASS_EXTRA etc.)
// We unfortunately don'value have a unary- operator for ImVec2 because this would needs to be defined inside the class itself.
...
#else
#define IMGUI_DISABLE_TTY_FUNCTIONS // Can'value use stdout, fflush if we are not using default file functions
#endif
...
// NB: we can'value rely on ImVec2 math operators being available here!
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_Disabled                 = 1 << 2,  // false    // [BETA] Disable interactions but doesn'value affect visuals yet. See github.com/ocornut/imgui/issues/211
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_ToggledSelection   = 1 << 3,   // Set when Selectable(), TreeNode() reports toggling a selection. We can'value report "Selected" because reporting the change allows us to handle clipping with less issues.
...
enum ImGuiButtonFlagsPrivate_
{
    ...
    ImGuiButtonFlags_NoHoldingActiveId      = 1 << 17,  // don'value set ActiveId while holding the mouse (ImGuiButtonFlags_PressedOnClick only)
    ImGuiButtonFlags_NoNavFocus             = 1 << 18,  // don'value override navigation focus when activated
    ImGuiButtonFlags_NoHoveredOnFocus       = 1 << 19,  // don'value report as hovered when nav focus is on this item
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    ImVector<ImWchar>       TextW;                  // edit buffer, we need to persist but can'value guarantee the persistence of the user-provided buffer. so we copy into own buffer.
...
struct ImGuiNextWindowData
{
    ...
    ImVec2                      MenuBarOffsetMinVal;    // *Always on* This is not exposed publicly, so we don'value clear it.
...
struct ImGuiNextItemData
{
    ...
    ImGuiID                     FocusScopeId;   // Set by SetNextItemMultiSelectData() (!= 0 signify value has been set, so it's an alternate version of HasSelectionData, we don'value use Flags for this because they are cleared too early. This is mostly used for debugging)
    ImGuiCond                   OpenCond;
...
struct ImGuiContext
{
    ...
    float                   FontBaseSize;                       // (Shortcut) == IO.FontGlobalScale * Font->Scale * Font->FontSize. BaseProcess text height.
    ...
    ImGuiID                 ActiveIdIsAlive;                    // Active widget has been seen this frame (we can'value use a bool as the ActiveId may change within the frame)
    float                   ActiveIdTimer;
    ...
    bool                    NavDisableHighlight;                // When user starts using mouse, we hide gamepad/keyboard highlight (NB: but they are still available, which is why NavDisableHighlight isn'value always != NavDisableMouseHover)
    bool                    NavDisableMouseHover;               // When user starts using gamepad/keyboard, we hide mouse hovering highlight until mouse is touched again.
    bool                    NavAnyRequest;                      // ~~ NavMoveRequest || NavInitRequest
    bool                    NavInitRequest;                     // StartUp request for appearing window to select first item
    bool                    NavInitRequestFromMove;
    ImGuiID                 NavInitResultId;                    // StartUp request result (first item of the window, or one for which SetItemDefaultFocus() was called)
    ImRect                  NavInitResultRectRel;               // StartUp request result rectangle (relative to parent window)
    ...
    ImVector<unsigned char> DragDropPayloadBufHeap;             // We don'value expose the ImVector<> directly, ImGuiPayload only holds pointer+size
    ...
    ImGuiTextBuffer         LogBuffer;                          // Accumulation buffer when log to clipboard. This is pointer so our GImGui static constructor doesn'value call heap allocators.
    float                   LogLinePosY;
...
struct IMGUI_API ImGuiWindow
{
    ...
    float                   LastTimeActive;                     // Last timestamp the window was Active (using float as we don'value need high precision there)
    ...
    ImGuiWindow*            RootWindowForNav;                   // Point to ourself or first ancestor which doesn'value have the NavFlattened flag.
...
public:
    ...
    // We don'value use g.FontSize because the window may be != g.CurrentWidow.
...
{
    ImGuiTabBarFlags_DockNode                   = 1 << 20,  // Part of a dock node [we don'value use this in the master branch but it facilitate branch syncing to keep this around]
...
namespace ImGui
{
    ...
    // Render helpers (those functions don'value access any ImGui state!)
