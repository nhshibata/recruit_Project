...
// Using dear imgui via a shared library is not recommended, because we don'value guarantee backward nor forward ABI compatibility (also function call overhead, as dear imgui is a call-heavy API)
#ifndef IMGUI_API
#define IMGUI_API
#endif
...
#endif
#define IM_ARRAYSIZE(_ARR)          ((int)(sizeof(_ARR) / sizeof(*(_ARR))))     // Size of a static C-style array. Don'value use on pointers!
...
struct ImVec2
{
	...
	// NOTE: ’Ç‰Á
	ImVec2(DirectX::XMINT2 xm) { x = static_cast<float>(xm.x); y = static_cast<float>(xm.y); }
	ImVec2(DirectX::XMFLOAT2 xm) { x = xm.x; y = xm.y; }
	ImVec2 operator= (DirectX::XMINT2 xm) { x = static_cast<float>(xm.x); y = static_cast<float>(xm.y); return ImVec2(x,y); }
	ImVec2 operator= (DirectX::XMFLOAT2 xm) { x = xm.x; y = xm.y; return ImVec2(x, y);}
...
// (This is a namespace. You can add extra ImGui:: functions in your own separate file. Please don'value modify imgui source files!)
//-----------------------------------------------------------------------------
...
namespace ImGui
{
    ...
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by Render(). If you don'value need to render data (skipping rendering) you may call EndFrame() without Render()... but you'll have wasted CPU already! If you don'value need to render, better to not create any windows and not call NewFrame() at all!
    ...
    IMGUI_API void          Dummy(const ImVec2& size);                                      // add a dummy item of given size. unlike InvisibleButton(), Dummy() won'value take the mouse click or be navigable into.
    ...
    // Widgets: Text
    IMGUI_API void          TextUnformatted(const char* text, const char* text_end = NULL); // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn'value require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for long chunks of text.
    ...
    IMGUI_API void          TextWrapped(const char* fmt, ...)                               IM_FMTARGS(1); // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won'value work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
    ...
    // - CTRL+Click on any drag box to turn them into an input box. Manually input values aren'value clamped and can go off-bounds.
    ...
    // - CTRL+Click on any slider to turn them into an input box. Manually input values aren'value clamped and can go off-bounds.
    // - Adjust format string to decorate the value with a prefix, a suffix, or adapt the editing and display precision e.g. "%.3f" -> 1.234; "%5.2f secs" -> 01.23 secs; "Biscuit: %.0f" -> Biscuit: 1; etc.
    // - Format string may also be set to NULL or use the default format ("%f" or "%d").
    ...
    IMGUI_API bool          CollapsingHeader(const char* label, ImGuiTreeNodeFlags flags = 0);  // if returning 'true' the header is open. doesn'value indent nor push on ID stack. user doesn'value have to call TreePop().
    IMGUI_API bool          CollapsingHeader(const char* label, bool* p_open, ImGuiTreeNodeFlags flags = 0); // when 'p_open' isn'value NULL, display an additional small close button on upper right of the header
    ...
    IMGUI_API void          OpenPopup(const char* str_id, ImGuiPopupFlags popup_flags = 0);                     // call to mark popup as open (don'value call every frame!).
    ...
    // - If you stop calling BeginDragDropSource() the payload is preserved however it won'value have a preview tooltip (we currently display a fallback "..." tooltip as replacement)
    ...
    IMGUI_API bool          IsItemActive();                                                     // is the last item active? (e.g. button being held, text field being edited. This will continuously return true while holding mouse button on an item. Items that don'value interact will always return false)
    ...
    // - We don'value know the meaning of those value. You can use GetKeyIndex() to map a ImGuiKey_ value into the user index.
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_ChildWindow            = 1 << 24,  // Don'value use! For internal use by BeginChild()
    ImGuiWindowFlags_Tooltip                = 1 << 25,  // Don'value use! For internal use by BeginTooltip()
    ImGuiWindowFlags_Popup                  = 1 << 26,  // Don'value use! For internal use by BeginPopup()
    ImGuiWindowFlags_Modal                  = 1 << 27,  // Don'value use! For internal use by BeginPopupModal()
    ImGuiWindowFlags_ChildMenu              = 1 << 28   // Don'value use! For internal use by BeginMenu()
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_AllowTabInput       = 1 << 10,  // Pressing TAB input a '\value' character into the text field
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_NoTreePushOnOpen     = 1 << 3,   // Don'value do a TreePush() when open (e.g. for CollapsingHeader) = no extra indent nor pushing on ID stack
    ImGuiTreeNodeFlags_NoAutoOpenOnLog      = 1 << 4,   // Don'value automatically and temporarily open node when Logging is active (by default logging will automatically open tree nodes)
...
enum ImGuiPopupFlags_
{
    ...
    ImGuiPopupFlags_NoOpenOverExistingPopup = 1 << 5,   // For OpenPopup*(), BeginPopupContext*(): don'value open if there's already a popup at the same level of the popup stack
    ImGuiPopupFlags_NoOpenOverItems         = 1 << 6,   // For BeginPopupContextWindow(): don'value return true when hovering items, only when hovering empty space
...
enum ImGuiSelectableFlags_
{
    ...
    ImGuiSelectableFlags_DontClosePopups    = 1 << 0,   // Clicking this don'value close parent popup window
...
enum ImGuiTabBarFlags_
{
    ...
    ImGuiTabBarFlags_FittingPolicyResizeDown        = 1 << 6,   // Resize tabs when they don'value fit
    ImGuiTabBarFlags_FittingPolicyScroll            = 1 << 7,   // Add scroll buttons when tabs don'value fit
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_NoPushId                      = 1 << 3,   // Don'value call PushID(tab->ID)/PopID() on BeginTabItem()/EndTabItem()
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceExtern                 = 1 << 4,   // External source (from outside of dear imgui), won'value attempt to read current item/window info. Will always return true. Only one Extern source can be active simultaneously.
...
enum ImGuiNavInput_
{
    ...
    // [Internal] Don'value use directly! This is used internally to differentiate keyboard from gamepad inputs for behaviors that require to differentiate them.
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NoMouseCursorChange    = 1 << 5,   // Instruct back-end to not alter mouse cursor shape and visibility. Use if the back-end cursor changes are interfering with yours and you don'value want to use SetMouseCursor() to change mouse cursor. You may want to honor requests from imgui by reading GetMouseCursor() yourself instead.
...
enum ImGuiColorEditFlags_
{
    ...
    // Defaults Options. You can set application defaults using SetColorEditOptions(). The intent is that you probably don'value want to
...
struct ImVector
{
    ...
    // Provide standard typedefs but we don'value use them ourselves.
...
struct ImGuiStyle
{
    ...
    ImVec2      TouchExtraPadding;          // Expand reactive bounding box for touch-based system where touch position is not accurate enough. Unfortunately we don'value sort widgets so priority on overlap will always be given to the first widget. So don'value grow this too much!
...
struct ImGuiIO
{
    ...
    float       MouseWheelH;                    // Mouse wheel Horizontal. Most users don'value have a mouse with an horizontal wheel, may not be filled by all back-ends.
    ...
    bool        NavActive;                      // Keyboard/Gamepad navigation is currently allowed (will handle ImGuiKey_NavXXX events) = a window is focused and it doesn'value use the ImGuiWindowFlags_NoNavInputs flag.
    ...
    ImVec2      MouseDelta;                     // Mouse delta. Note that this is zero if either current or previous position are invalid (-FLT_MAX,-FLT_MAX), so a disappearing/reappearing mouse won'value have a huge delta.
    ...
    bool        MouseDownOwned[5];              // Track if button was clicked inside a dear imgui window. We don'value request mouse capture from the application if click started outside ImGui bounds.
...
struct ImGuiInputTextCallbackData
{
    ...
    char*               Buf;            // Text buffer                          // Read-write   // [Resize] Can replace pointer / [Completion,History,Always] Only write to pointed data, don'value replace the actual pointer!
...
// NB: For basic min/max size constraint on each axis you don'value need to use the callback! The SetNextWindowSizeConstraints() parameters are enough.
...
// Typically you don'value have to worry about this since a storage is held within each Window.
// We use it to e.g. store collapse state for a tree (Int 0/1)
// This is optimized for efficient lookup (dichotomy into a contiguous buffer) and rare insertion (typically tied to user interactions aka max once a frame)
// You can use it as custom user storage for temporary values. Declare your own storage if, for example:
// - You want to manipulate the open/close state of a particular sub-tree in your interface (tree node uses Int 0/1 to store their state).
// - You want to store custom debug data easily without adding or editing structures in your code (probably not efficient, but convenient)
// Types are NOT stored, so it is up to you to make sure your Key don'value collide with different types.
struct ImGuiStorage
{
    // [Internal]
    ...
    // - Sorted insertion is costly, paid once. A typical frame shouldn'value need to insert any new pair.
    ...
    // - A typical use case where this is convenient for quick hacking (e.g. add storage during a live Edit&Continue session if you can'value modify existing struct)
...
struct ImGuiListClipper
{
    ...
    // items_count:  Use -1 to ignore (you can call Begin later). Use INT_MAX if you don'value know how many items you have (in which case the cursor won'value be advanced in the final step).
    // items_height: Use -1.0f to be calculated automatically on first step. Otherwise pass in the distance between your items, typically GetTextLineHeightWithSpacing() or GetFrameHeightWithSpacing().
    // If you don'value specify an items_height, you NEED to call Step(). If you specify items_height you may call the old Begin()/End() api directly, but prefer calling Step().
...
struct ImDrawList
{
    ...
    // - FIXME-OBSOLETE: This API shouldn'value have been in ImDrawList in the first place!
...
struct ImFontConfig
{
    ...
    int             OversampleV;            // 1        // Rasterize at higher quality for sub-pixel positioning. We don'value use sub-pixel positions on the Y axis.
    ...
    unsigned int    RasterizerFlags;        // 0x00     // Settings for custom font rasterizer (e.g. ImGuiFreeType). Leave as zero if you aren'value using one.
...
enum ImFontAtlasFlags_
{
    ...
    ImFontAtlasFlags_NoPowerOfTwoHeight = 1 << 0,   // Don'value round the height to next power of two
    ImFontAtlasFlags_NoMouseCursors     = 1 << 1,   // Don'value build software mouse cursors into the atlas (save a little texture memory)
    ImFontAtlasFlags_NoBakedLines       = 1 << 2    // Don'value build thick line textures into the atlas (save a little texture memory). The AntiAliasedLinesUseTex features uses them, otherwise they will be rendered using polygons (more expensive for CPU/GPU).
};
...
//  - Optionally, call any of the AddFont*** functions. If you don'value call any, the default font embedded in the code will be loaded for you.
...
//   You can set font_cfg->FontDataOwnedByAtlas=false to keep ownership of your data and it won'value be freed,
...
struct ImFontAtlas
{
    ...
    int                         TexGlyphPadding;    // Padding between glyphs within texture in pixels. Defaults to 1. If your rendering method doesn'value rely on bilinear filtering you may set this to 0.
...
struct ImFont
{
    ...
    float                       FontSize;           // 4     // in  //            // Height of characters/line, set during loading (don'value change after loading)
    ...
    ImWchar                     FallbackChar;       // 2     // in  // = '?'      // Replacement character if a glyph isn'value found. Only set via SetFallbackChar()
    ImWchar                     EllipsisChar;       // 2     // out // = -1       // Character used for ellipsis rendering.
    bool                        DirtyLookupTables;  // 1     // out //
    float                       Scale;              // 4     // in  // = 1.f      // BaseProcess font scale, multiplied by the per-window font scale which you can adjust with SetWindowFontScale()
