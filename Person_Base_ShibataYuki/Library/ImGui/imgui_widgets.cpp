...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
        ...
        // - We also don'value vertically center the text within the line full height, which is unlikely to matter because we are likely the biggest and only item on the line.
        // - We use memchr(), pay attention that well optimized versions of those str/mem functions are much faster than a casually written loop.
        const char* line = text;
        const float line_height = GetTextLineHeight();
        ImVec2 text_size(0, 0);

        // Lines to skip (can'value skip when logging text)
        ImVec2 pos = text_pos;
        if (!g.LogEnabled)
        {
    ...
    else
    {
        ...
        // Render (we don'value hide text after ## in this end-user function)
...
void ImGui::BulletTextV(const char* fmt, va_list args)
{
    ...
    const ImVec2 total_size = ImVec2(g.FontSize + (label_size.x > 0.0f ? (label_size.x + style.FramePadding.x * 2) : 0.0f), label_size.y);  // Empty text doesn'value add padding
...
// - PressedOnDoubleClick can be associated by PressedOnClickRelease/PressedOnRelease, in which case the second release event won'value be reported.
//------------------------------------------------------------------------------------------------------------------------------------------------
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    // Drag source doesn'value report as hovered
    ...
    // We report navigated item as hovered but we don'value set g.HoveredId to not interfere with mouse.
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    if ((flags & ImGuiButtonFlags_AlignTextBaseLine) && style.FramePadding.y < window->DC.CurrLineTextBaseOffset) // Try to vertically align buttons that are smaller/have no padding so that text baseline matches (bit hacky, since it shouldn'value be a flag)
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
    ...
    // (this isn'value the regular behavior of buttons, but it doesn'value affect the user much because navigation tends to keep items visible).
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    {
        // Vertical separator, for menu bars (use current line height). Not exposed because it is misleading and it doesn'value have an effect on regular layout.
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        // We don'value provide our width to the layout so that it doesn'value get feed back into AutoFit
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    IM_ASSERT((flags & (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)) != (ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_NoPreview)); // Can'value use both flags together
    ...
    // We don'value use BeginPopupEx() solely because we have a custom name string, which we could make an argument to BeginPopupEx()
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    // The old Combo() API exposed "popup_max_height_in_items". The new more general BeginCombo() API doesn'value have/need it, but we emulate it here.
...
{
    // Signedness doesn'value matter when pushing integer arguments
...
bool ImGui::DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    // We don'value support '-' op because it would conflict with inputing negative value.
    ...
    {
        // For floats we have to ignore format with precision (e.g. "%.2f") because sscanf doesn'value take them in
        format = "%f";
    ...
    else if (data_type == ImGuiDataType_U32 || data_type == ImGuiDataType_S64 || data_type == ImGuiDataType_U64)
    {
        ...
        // We don'value bother handling support for legacy operators since they are a little too crappy. Instead we will later implement a proper expression evaluator in the future.
...
TYPE ImGui::RoundScalarWithFormatT(const char* format, ImGuiDataType data_type, TYPE v)
{
    ...
    if (fmt_start[0] != '%' || fmt_start[1] == '%') // Don'value apply if the value is not visible in the format string
        return v;
    char v_str[64];
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float zero_deadzone_halfsize = 0.0f; // Drag widgets have no deadzone (as it doesn'value make sense)
    if (is_logarithmic)
    {
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        else
        {
            ...
            float t_with_flip = flipped ? (1.0f - t) : t; // value, but flipped if necessary to account for us flipping the range
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Nav)
        {
            ...
            else if (g.SliderCurrentAccumDirty)
            {
                ...
                {
                    set_new_value = false;
                    g.SliderCurrentAccum = 0.0f; // If pushing up against the limits, don'value continue to accumulate
                }
                else
                {
                    set_new_value = true;
...
// It would be possible to lift that limitation with some work but it doesn'value seem to be worth it for sliders.
...
// We don'value use strchr() because our strings are usually very short and often start with '%'
...
{
    // Printf/scanf types modifiers: I/L/h/j/l/value/w/z. Other uppercase letters qualify as types aka end of the format.
    if (fmt[0] != '%')
        return fmt;
...
const char* ImParseFormatTrimDecorations(const char* fmt, char* buf, size_t buf_size)
{
    ...
    if (fmt_end[0] == 0) // If we only have leading decoration, we don'value need to copy the data.
        return fmt_start;
...
// FIXME: This is still used by some navigation code path to infer a minimum tweak step, but we should aim to rework widgets so it isn'value needed.
...
// We don'value use an enum so we can build even with conflicting symbols (if another user of stb_textedit.h leak their STB_TEXTEDIT_K_* symbols)
...
static void stb_textedit_replace(STB_TEXTEDIT_STRING* str, STB_TexteditState* state, const STB_TEXTEDIT_CHARTYPE* text, int text_len)
{
    ...
    IM_ASSERT(0); // Failed to insert character, normally shouldn'value happen because of how we currently use stb_textedit_replace()
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackHistory) && (flags & ImGuiInputTextFlags_Multiline)));        // Can'value use both together (they both use up/down keys)
    IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackCompletion) && (flags & ImGuiInputTextFlags_AllowTabInput))); // Can'value use both together (they both use tab key)
    ...
    {
        // Access state even if we don'value own it yet.
        ...
        state->CurLenA = (int)(buf_end - buf);      // We can'value get the result from ImStrncpy() above because it is not UTF-8 aware. Here we'll cut off malformed UTF-8.
    ...
    if (g.ActiveId != id && init_make_active)
    {
        ...
        if (flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_AllowTabInput))  // Disable keyboard tabbing out as we will use the \value character.
            g.ActiveIdUsingKeyInputMask |= ((ImU64)1 << ImGuiKey_Tab);
    }

    // We have an edge case if ActiveId was set through another widget (e.g. widget being swapped), clear id immediately (don'value wait until the end of the function)
    ...
    if (g.ActiveId == id)
    {
        ...
        // Although we are active we don'value prevent mouse from hovering other elements unless we are interacting right now with the widget.
        ...
        // It is ill-defined whether the back-end needs to send a \value character when pressing the TAB keys.
    ...
    if (g.ActiveId == id)
    {
        ...
        // When using 'ImGuiInputTextFlags_EnterReturnsTrue' as a special case we reapply the live buffer back to the input buffer before clearing ActiveId, even though strictly speaking it wasn'value modified on this frame.
        // If we didn'value do that, code like InputInt() with ImGuiInputTextFlags_EnterReturnsTrue would fail.
        // This also allows the user to use InputText() with ImGuiInputTextFlags_EnterReturnsTrue without maintaining any user-side storage (please note that if you use this property along ImGuiInputTextFlags_CallbackResize you can end up with your temporary string object unnecessarily allocating once a frame, either store your string data, either if you don'value then don'value use ImGuiInputTextFlags_CallbackResize).
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            // Store text height (note that we haven'value calculated text width at all, see GitHub issues #383, #1224)
            if (is_multiline)
        ...
        if (render_selection)
        {
            ...
            float bg_offy_up = is_multiline ? 0.0f : -1.0f;    // FIXME: those offsets should be part of the style? they don'value play so well with multi-line selection.
    ...
    else
    {
        ...
        if (is_multiline)
            text_size = ImVec2(inner_size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_display_end) * g.FontSize); // We don'value need width
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            if (ColorEdit4("##rgb", col, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
                ...
                // For the later we don'value want to run the hue-wrap canceling code. If you are well versed in HSV picker please provide your input! (See #2050)
...
// 'desc_id' is not called 'label' because we don'value display it next to the button, but only in the tooltip.
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    {
        // Because GetColorU32() multiplies by the global style Alpha and we don'value want to display a checkerboard if the source code had no alpha
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        if (g.LogEnabled)
        {
            // NB: '##' is normally used to hide text (as a library-wide feature), so we need to specify the text range to make sure the ## aren'value stripped out here.
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Update NavId when clicking or when Hovering (this doesn'value happen on most widgets), so navigation can be resumed with gamepad/keyboard
...
bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items)
{
    ...
    // We don'value add this extra bit if items_count <= height_in_items. It is slightly dodgy, because it means a dynamic list of items will make the widget resize occasionally when it crosses that size.
    ...
    // We include ItemSpacing.y so that a list sized for the exact number of items doesn'value make a scrollbar appears. We could also enforce that by passing a flag to BeginChild().
    ImVec2 size;
    size.x = 0.0f;
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ...
    ImGuiContext& g = *GImGui;
    bool value_changed = false;
    ImGuiListClipper clipper(items_count, GetTextLineHeightWithSpacing()); // We know exactly our line height here so we pass it as a minor optimization, but generally you don'value need to.
    while (clipper.Step())
...
bool ImGui::BeginMenuBar()
{
    ...
    // We don'value clip with current window clipping rectangle as it is already set to the area below. However we clip with window full rect.
    // We remove 1 worth of rounding to Max.x to that text in long menus and small windows don'value tend to display over the lower-right rounded area, which looks particularly glitchy.
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.ParentLayoutType == ImGuiLayoutType_Horizontal && g.NavMoveRequestForward == ImGuiNavForward_None)
        {
            ...
            // This involve a one-frame delay which isn'value very problematic in this situation. We could remove it by scoring in advance for multiple window (probably not worth the hassle/cost)
            const ImGuiNavLayer layer = ImGuiNavLayer_Menu;
            IM_ASSERT(window->DC.NavLayerActiveMaskNext & (1 << layer)); // Sanity check
            FocusWindow(window);
            SetNavIDWithRectRel(window->NavLastIds[layer], layer, 0, window->NavRectRel[layer]);
            g.NavLayer = layer;
            g.NavDisableHighlight = true; // Hide highlight for the current frame so we don'value see the intermediary selection.
...
void ImGui::EndMainMenuBar()
{
    ...
    // FIXME: With this strategy we won'value be able to restore a NULL focus.
    ImGuiContext& g = *GImGui;
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (menuset_is_open)
        g.NavWindow = window;  // Odd hack to allow hovering across menus of a same menu-set (otherwise we wouldn'value be able to hover parent)
    ...
    {
        // Don'value recycle same menu level in the same frame, first close the other menu and yield for a frame.
        OpenPopup(label);
        return false;
    }
...
void ImGui::EndMenu()
{
    ...
    // A menu doesn'value close itself because EndMenuBar() wants the catch the last Left<>Right inputs.
    // However, it means that with the current code, a BeginMenu() from outside another menu or a menu-bar won'value be closable with the Left direction.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        // Additionally, when using TabBarAddTab() to manipulate tab bar order we occasionally insert new tabs that don'value have a width yet,
    ...
    {
        // If we don'value have enough room, resize down the largest tabs first
    ...
    // Actual layout in host window (we don'value do it in BeginTabBar() so as not to waste an extra frame)
    ImGuiWindow* window = g.CurrentWindow;
...
void ImGui::TabBarCloseTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab)
{
    if ((tab_bar->VisibleTabId == tab->ID) && !(tab->Flags & ImGuiTabItemFlags_UnsavedDocument))
    {
        ...
        // However we don'value run it in the case where the 'Unsaved' flag is set, so user gets a chance to fully undo the closure
...
{
    ImGuiContext& g = *GImGui;
    float margin = g.FontSize * 1.0f; // When to scroll to make Tab N+1 visible always make a bit of N visible to suggest more scrolling area (since we don'value have a scrollbar)
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // If the user called us with *p_open == false, we early out and don'value render.
    // We make a call to ItemAdd() so that attempts to use a contextual popup menu with an implicit ID won'value use an older ID.
    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.LastItemStatusFlags);
    if (p_open && !*p_open)
    {
    ...
    // We don'value have CPU clipping primitives to clip the CloseButton (until it becomes a texture), so need to add an extra draw call (temporary in the case of vertical animation)
    ...
    // Allow the close button to overlap unless we are dragging (in which case we don'value want any overlapping tabs to be hovered)
    if (!held)
        SetItemAllowOverlap();
    ...
    // Tooltip (FIXME: Won'value work over the close button because ItemOverlap systems messes up with HoveredIdTimer)
...
{
    // We have few columns per window so for now we don'value need bother much with turning this into a faster lookup.
...
ImGuiID ImGui::GetColumnsID(const char* str_id, int columns_count)
{
    ...
    // In addition, when an identifier isn'value explicitly provided we include the number of columns in the hash to make it uniquer.
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiColumnsFlags flags)
{
    ...
    // We don'value generally store Indent.x inside ColumnsOffset because it may be manipulated by the user.
...
void ImGui::EndColumns()
{
    ...
    if (!(flags & ImGuiColumnsFlags_GrowParentContentsSize))
        window->DC.CursorMaxPos.x = columns->HostCursorMaxPosX;  // Restore cursor max pos, as columns don'value grow parent
